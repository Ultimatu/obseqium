<div>
    ...
    <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        @if ($booked)
            <div class="bg-brand-50 border border-brand-200 rounded-2xl p-8 text-center" x-data x-init="$el.animate([{ opacity: 0, transform: 'scale(0.95)' }, { opacity: 1, transform: 'scale(1)' }], { duration: 300, easing: 'ease-out' })">
                ...
                <p class="text-brand-700">Votre demande de rendez-vous a été reçue. Nous vous contacterons rapidement
                    pour confirmer le créneau.</p>
            </div>
        @else
            ...
            <div class="flex items-start w-full mb-10">
                ...
                @foreach ($steps as $i => $label)
                    <div class="flex flex-col items-center shrink-0">
                        ...
                        <span
                            class="text-xs font-medium mt-2 text-center hidden sm:block
                    ...
                    @if ($i < count($steps) - 1)
                        <div
                            class="flex-1 h-0.5 mt-4 mx-2 transition-colors duration-500 {{ $step > $i + 1 ? 'bg-brand-600' : 'bg-gray-200' }}">
                        </div>
                    @endif
                @endforeach
            </div>
            ...
            <div wire:key="step-{{ $step }}" x-data x-init="$el.animate([{ opacity: 0, transform: 'translateX(16px)' }, { opacity: 1, transform: 'translateX(0)' }], { duration: 220, easing: 'ease-out' })"
                        ...
                        <div class="grid sm:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input wire:model="guest_name" type="text"
                                    class="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-transparent transition">
                                @error('guest_name')
                            ...
                            <div>
                                ...
                                <input wire:model="guest_company" type="text"
                                    class="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-transparent transition">
                            </div>
                        </div>
                        <div class="flex justify-end pt-2">
                            <button wire:click="nextStep"
                                class="bg-brand-600 hover:bg-brand-700 text-white font-semibold px-6 py-3 rounded-xl transition-colors flex items-center gap-2">
                ...
                @if ($step === 2)
                    ...
                    <div class="space-y-5">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-3">Format souhaité <span
                                    class="text-red-500">*</span></label>
                            <div class="grid grid-cols-2 gap-3">
                                <!-- Visioconférence -->
                                <button type="button" wire:click="$set('type', 'visio')"
                                ...
                                {{ $type === 'visio' ? 'border-brand-600 bg-brand-50 shadow-sm' : 'border-gray-200 hover:border-brand-300 hover:bg-gray-50' }}">
                                    <svg class="w-7 h-7 {{ $type === 'visio' ? 'text-brand-600' : 'text-gray-400' }} transition-colors"
                                        fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8"
                                            d="M15 10l4.553-2.069A1 1 0 0121 8.82v6.36a1 1 0 01-1.447.894L15 14M3 8a2 2 0 012-2h8a2 2 0 012 2v8a2 2 0 01-2 2H5a2 2 0 01-2-2V8z" />
                                    </svg>
                                    <span
                                        class="font-semibold text-sm {{ $type === 'visio' ? 'text-brand-700' : 'text-gray-600' }}">Visioconférence</span>
                                    <span
                                        class="text-xs {{ $type === 'visio' ? 'text-brand-500' : 'text-gray-400' }}">Teams,
                                        Zoom, Meet…</span>
                                    @if ($type === 'visio')
                                        <div
                                ...
                                <!-- Présentiel -->
                                <button type="button" wire:click="$set('type', 'presential')"
                                    class="relative flex flex-col items-center gap-2 border-2 rounded-xl p-5 cursor-pointer transition-all duration-150
                                {{ $type === 'presential' ? 'border-brand-600 bg-brand-50 shadow-sm' : 'border-gray-200 hover:border-brand-300 hover:bg-gray-50' }}">
                                    <svg class="w-7 h-7 {{ $type === 'presential' ? 'text-brand-600' : 'text-gray-400' }} transition-colors"
                                        fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8"
                                    ...
                                    <span
                                        class="font-semibold text-sm {{ $type === 'presential' ? 'text-brand-700' : 'text-gray-600' }}">Présentiel</span>
                                    <span
                                        class="text-xs {{ $type === 'presential' ? 'text-brand-500' : 'text-gray-400' }}">Dans
                                        nos locaux</span>
                                    @if ($type === 'presential')
                                        <div
                                            class="absolute top-2 right-2 w-5 h-5 bg-brand-600 rounded-full flex items-center justify-center">
                        ...
                        <div>
                            ...
                            <textarea wire:model="subject" rows="4" placeholder="Décrivez brièvement le sujet de votre rendez-vous…"
                                class="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-transparent transition resize-none"></textarea>
                        </div>

                        <div class="flex justify-between pt-2">
                            <button wire:click="prevStep"
                                class="text-gray-500 hover:text-gray-700 font-medium px-4 py-3 rounded-xl transition-colors flex items-center gap-2">
                                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="border border-brand-50 rounded-xl overflow-hidden">
                            <div class="flex flex-col sm:flex-row">
                                ...
                                {{-- Calendar panel --}}
                                <div class="p-5 sm:w-72 shrink-0 border-b sm:border-b-0 sm:border-r border-brand-50">
                                    {{-- Month navigation --}}
                                            ...
                                            @foreach ($timeSlots as $slot)
                                                ...
                                                @elseif ($selectedTime === $slot)
                                                    <div
                                                        ...
                                                        <svg class="w-4 h-4 text-accent-600 shrink-0" fill="none"
                                                            viewBox="0 0 24 24" stroke="currentColor">
                        ...
                        <div class="flex justify-between pt-2">
                            ...
                            <button wire:click="book"
                                class="bg-accent-600 hover:bg-accent-400 text-white font-semibold px-6 py-3 rounded-xl transition-colors shadow-sm flex items-center gap-2 active:scale-95">
                                ...
                                <svg class="w-4 h-4" wire:loading.remove wire:target="book" fill="none"
                                    viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M17 8l4 4m0 0l-4 4m4-4H3" />
                                </svg>
