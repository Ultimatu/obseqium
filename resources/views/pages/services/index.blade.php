<x-layouts.app title="Nos services QHSE">
    <div class="bg-gray-50 border-b border-gray-100 py-12">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <p class="text-gray-500 mt-2 max-w-xl">Une offre complète pour répondre à tous vos besoins en matière de
                QHSE.</p>
        </div>
    </div>
    ...
    <!-- Filter tabs -->
    @if (request('type'))
        <div class="bg-white border-b border-gray-100 sticky top-18.25 z-40">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
            @if ($services->isEmpty())
                <div class="text-center py-20 text-gray-400">
                    <svg class="w-12 h-12 mx-auto mb-4 text-gray-200" fill="none" viewBox="0 0 24 24"
                        stroke="currentColor">
                ...
                <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
                    @foreach ($services as $service)
                        <a href="{{ route('services.show', $service->slug) }}" data-animate="fade-up"
                            ...
                            @else
                                <div
                                    class="aspect-video bg-linear-to-br from-brand-50 to-brand-100 flex items-center justify-center relative overflow-hidden">
                                    <div
                                        class="absolute inset-0 bg-brand-500/5 group-hover:bg-brand-500/10 transition-colors">
                                    </div>
                                    @if ($service->icon)
                                        <span
                                            class="text-5xl group-hover:scale-110 transition-transform duration-300">{{ $service->icon }}</span>
                                    @endif
                                </div>
                            @endif
                            <div class="p-6 flex-1 flex flex-col relative">
                                <div class="mb-3">
                                    <span
                                        class="inline-block text-xs font-semibold uppercase tracking-wider px-2.5 py-1 rounded-full {{ $typeBadge[$service->type] ?? 'bg-gray-100 text-gray-600' }} group-hover:shadow-sm transition-shadow">
                                        {{ $typeLabel[$service->type] ?? ucfirst($service->type ?? '') }}
                                    </span>
                                </div>
                                <h2
                                    class="font-semibold text-gray-900 mb-2 group-hover:text-brand-600 transition-colors text-lg">
                                    {{ $service->title }}</h2>
                                <p class="text-gray-600 text-sm leading-relaxed flex-1 line-clamp-4">
                                    {{ $service->description }}</p>
                                <div
                                    class="mt-5 flex items-center gap-1 text-brand-600 text-sm font-medium icon-bounce">
                                    En savoir plus
    ...
    {{-- CTA --}}
    <x-cta-banner title="Besoin d’une prestation sur mesure ?"
        description="Nous étudions chaque demande pour vous proposer la solution la plus adaptée."
        primaryLabel="Demander un devis gratuit" primaryRoute="quotes.request" secondaryLabel="Nous contacter"
